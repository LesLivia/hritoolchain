...
plt.ylabel(r'$(p\_min, p\_max) [\%]$', fontsize=22, weight='bold')
...
ax[0].vlines([t_rec[0]], [min(y)], [max(y)], colors='red', linewidth=2)
...
p4, p5 = ax[1].plot(x, est_values, 'k--',
                    ...
                    linewidth=3)
ax[1].vlines([t_rec[0]], [min(mnf_ftg)], [max(mnf_ftg)], colors='red', linewidth=2)
ax[1].legend((p2, p4), ('$\mathrm{ln}(f_\mathrm{MEAN}(B(t)))$', r'Regression Line',), loc='lower right', shadow=False)
...
ax[2].vlines([t_rec[0]], [min(ftg)], [max(ftg)], colors='red', linewidth=2)
ax[2].text(120, 0.075, '$1-e^{-\hat{\lambda}t}$', fontsize=35, backgroundcolor='white')
ax[2].text(420, 0.15, r'$F(296.3)e^{-\hat{\rho}(t-296.3)}$', fontsize=30, backgroundcolor='white')
ax[2].legend((p6), (r'$F(t)$',), loc='lower right', shadow=False)

plt.tight_layout()
plt.savefig('/Users/lestingi/Desktop/phd-workspace/_papers/RA-L_IROS21/journal_version/IEEETransactions_LaTeX/IEEEtran/figs/emg_plots.pdf', dpi=DPI)
